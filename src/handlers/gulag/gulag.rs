use crate::{
    ...
    handlers::eventhandler::HandlerResponse,
};
...
pub fn send_error(err: &str) -> HandlerResponse {
    HandlerResponse {
...
pub async fn member_has_role(
    ...
    match find_role(http, guildid, role_name).await {
        Some(derpies_role) => {
...
pub async fn find_gulag_role(http: &Arc<Http>, guildid: u64) -> Option<Role> {
    find_role(http, guildid, "gulag").await
}
...
pub async fn send_to_gulag_and_message(
    ...
    let gulag_role = find_gulag_role(http, guildid)
        .await
        .with_context(|| "Couldn't find gulag role".to_string())?;
    let gulaglength = 300;

    let gulag_channel = find_channel(http, guildid, "the-gulag".to_string())
        .await
        .with_context(|| "Cant find gulag channel".to_string())?;
    let gulag_user = add_to_gulag(
        http,
        guildid,
        userid,
        gulag_role.id.0,
...
async fn remove_from_gulag(
    ...
    let channel = find_channel(&http, guildid, "the-gulag".to_string())
        .await
                    ...
                    .await
                    {
                        Ok(_) => {
                            ...
                            if result.message_id != 0 {
                                ...
                                let done_result: MessageVotes = diesel::update(
                                    message_votes
                                        .filter(message_votes::message_id.eq(result.message_id)),
                                )
                                .set(message_votes::job_status.eq(JobStatus::Done))
                                .get_result(conn)
                                .with_context(|| {
                                    format!("failed to done message_vote_id {}", result.message_id)
                                })
                                .unwrap();
...
pub fn run_gulag_vote_check(http: &Arc<Http>) {
    ...
    spawn(async move {
        ...
        loop {
            ...
            if !results.is_empty() {
                for result in results {
                    if let Err(err) = gulag_check_handler(http.to_owned(), conn, &result).await {
...
async fn gulag_check_handler(
    ...
    let updated_result: MessageVotes = diesel::update(message_votes.find(result.message_id))
        .set((
            ...
            message_votes::total_vote_tally.eq(result.current_vote_tally + result.total_vote_tally),
