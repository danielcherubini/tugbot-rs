...
pub async fn setup_interaction(
    ...
    match gulag_spam_detection(requesterid, conn).await {
        Ok(_) => {
                        ...
                        if gulag::is_tugbot(&ctx.http, user).await.unwrap() {
                            ...
                            match gulag::find_gulag_role(&ctx.http, *guildid.as_u64()).await {
                                ...
                                Some(role) => {
                                    ...
                                    let jury_duty_role =
                                        find_jury_duty_role(&ctx.http, guildid.0).await.unwrap();
...
pub async fn process_gulag_votes(http: &Arc<Http>, channelid: u64, messageid: u64) -> Result<bool> {
